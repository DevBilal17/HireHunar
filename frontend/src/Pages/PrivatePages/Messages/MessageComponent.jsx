...
export default function MessageComponent() {
    ...
    return (
        <div className="flex h-full w-full bg-white relative">
            {/* Sidebar */}
            <div ref={slideRef} className={` md:flex w-1/4 flex-col border-r bg-white ${!slide ? 'lg:translate-x-0 -translate-x-[100%] ' : '-translate-x-0 w-[235px]'} transition-all duration-200 h-full  lg:relative absolute left-0 top-0`}>
                ...
                <div className="overflow-y-auto">
                    {initialMessages.map((msg) => (
                        <div
                            key={msg.id}
                            className={`flex items-center gap-3 p-4 hover:bg-gray-100 cursor-pointer border-b ${selectedMessage.id === msg.id ? "bg-blue-100" : ""
                                }`}
            ...
            <div className="flex-1 flex flex-col">
                <div className="p-4 flex items-center justify-between border-b">
                    ...
                    <button ref={buttonRef} className={`lg:hidden block $ text-3xl`} onClick={() => {setSlide(!slide); setIsOpen(!isOpen);}}>{isOpen ? (<RxCross2 />) : (<GiHamburgerMenu />)}</button>
                </div>
                <div className="flex-1 p-4 overflow-y-auto">
                    {selectedMessage.message.map((m, idx) => (
                        <div
                            key={idx}
                            className={`flex mb-4 max-w-xl ${m.from === "You" ? "self-end justify-end ml-auto text-right " : "text-left"}`}>
                            <p className={`p-3 rounded-xl shadow text-sm text-left ${m.from === "You" ? "bg-amber-600" : "bg-gray-100"}`}>{m.text}</p>
                        </div>
                    ))}
                </div>
